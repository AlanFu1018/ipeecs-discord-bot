...
- 每輪對話（含第一則訊息）皆先呼叫一次輕量 Prompt（`temperature=0`），結合對話歷史輸出三項結果：
  ...
  - **`QUERY`（Query Condensing）**：將代名詞／省略問題（例如：「那大二的必修呢？」）改寫為獨立、語意完整的**繁體中文**檢索問句（如：「113學年度資訊工程專長大二必修課程有哪些？」）。無論提問語言為何，檢索一律以繁體中文進行，以對應中文規章。
    - 輸入或歷史中出現的課號（如 `CE2003`）必須原樣保留，不得翻譯、省略或改寫成課名；程式層另會比對原始輸入與改寫結果，若課號遺失則自動補回檢索問句末尾。
...
- **向量資料庫**：
  ...
  - 檢索取前 `top_k`（預設 9）個片段。
- **混合檢索（課號關鍵字 + 語意向量）**：
  - **動機**：課號（如 `CE2003`、`EE3046`）在 Dense Embedding 中幾乎不帶語意，只寫課號的提問單靠向量相似度常檢索不到含該課號的課規表格。
  - **課號擷取**：`VectorStore.extract_course_codes()` 以正規表示式 `(?<![A-Za-z])([A-Za-z]{2,4})\s?(\d{4})(?!\d)` 擷取「2～4 個英文字母 + 4 位數字」，允許中間一個空白，並統一為大寫、去除重複（例：`ce 2003` → `CE2003`）。以前後文檢查（lookaround）取代 `\b`，使緊鄰中文的課號（「課程CE2003是」）也能擷取。
  - **關鍵字檢索**：對每個課號以 ChromaDB `where_document={"$contains": 課號}` 篩選含該課號的 Chunk，再依向量相似度排序；關鍵字結果最多占 `top_k` 的一半（多個課號平均分配），排在最前面。
  - **語意補足**：其餘名額以一般向量檢索補滿，並依 Chunk ID 去除重複。問句無課號、或關鍵字檢索失敗時，行為與純向量檢索相同。
  - **已知限制**：格式相同的非課號字串（如 `ISO 9001`）會被誤判，但因查無含該字串的 Chunk 而不影響結果；原始資料中 OCR 錯誤的課號（如 `C03025`）無法被比對。
...
│   │   └── vector_store.py  # ChromaDB 向量寫入、分區清除與混合檢索（課號關鍵字 + 語意向量）
│   ├── services/            # 業務邏輯與狀態管理
...
[chat_service.py → llm_gemini.py] 前置分析：LANGUAGE / QUERY（繁中獨立問句）/ SCOPE
           ...
           ▼
[chat_service.py] 檢查課號：原始輸入中有、改寫後遺失的課號補回檢索問句
           │
           ▼
[vector_store.py] 以 embed_gemini.py 向量化問句，於 ChromaDB 混合檢索 Top-9 相關段落
           ├── 問句含課號 ──> 先取含該課號的段落（最多 Top-k 一半，依相似度排序）
           └── 其餘名額 ──> 語意向量檢索補滿（去除重複）
           ├── 無結果 ──> Fallback 訊息
           ▼
...
3. 實作 `services/chat_service.py`：
   ...
   - **檢索模組**：向 ChromaDB 查詢最相關之規章段落（課號關鍵字 + 語意向量混合檢索）。
...
4. LLM 503 指數退避重試。
5. 課號混合檢索：只提及課號的問句也能檢索到對應課程（改寫保留課號 + ChromaDB `$contains` 關鍵字篩選）。
...
5. 修正原始資料中 OCR 錯誤的課號（如 `C03025` → `CO3025`），或於解析階段正規化。
6. 評估以 BM25 等稀疏檢索擴充混合檢索，涵蓋課名、法規條號等其他關鍵字型提問。
