...
- **資料來源**：
  ...
  - 關聯 PDF 文件：爬蟲自動下載網頁中連結的規章 PDF，或由管理者手動放置至 `data/raw/`。
- **同步腳本（`sync_data.py`）**：
  - 獨立於機器人主程式，手動或透過排程執行。
  - 網頁轉為 Markdown 儲存於 `data/markdown/`。
...
1. 實作 `src/services/crawler.py`：讀取 `urls.txt`，萃取主要正文轉換為 Markdown，並過濾出 PDF 檔案下載至 `data/raw/`。
2. 實作 `src/rag/parser.py`：清洗文字、去除多餘空行，將長文本切分為 500~800 字元的語意段落（Chunk），保留檔案來源 metadata。
3. 實作 `src/adapters/embed_gemini.py` 與 `src/rag/vector_store.py`：將文本塊批次轉換為向量，寫入本地 ChromaDB。
4. 撰寫 `sync_data.py` 整合以上流程，達成「一鍵同步與建庫」。

### 【階段三：Adapter 抽換層與對話核心邏輯（`services/`）】
1. 定義 `BaseLLMProvider` 與 `BaseEmbeddingProvider` 抽象介面，實作 `GeminiLLMProvider` 與 `GeminiEmbeddingProvider`。
2. 實作 `src/services/session.py`：字典維護 `user_id -> List[Message]`，設定滑動視窗上限與過期清理機制。
3. 實作 `src/services/chat_service.py`：
