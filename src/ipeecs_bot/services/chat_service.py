...
class ChatService:
    ...
    async def rewrite_query(self, session: UserSession, current_query: str) -> str:
        ...
        prompt = (
            ...
            f"{current_query}\n\n"
            "【任務】\n"
            f"結合對話歷史以以下格式輸出: 提問者的身分訊息 想要詢問 {current_query}"
            # "1. 歷史對話僅用於提取遺漏的主詞（例如：學年度、系所名稱、對象身分）。"
            # "2. 禁止合併舊話題(重要)：請勿將上一輪對話中提及的硬塞進新問句中。"
            # "3. 新問句的主體必須完全聚焦在【最新提問】的核心標的。"
    ...
    def build_system_prompt(self, context_docs: List[Dict[str, Any]]) -> str:
        ...
        return (
            ...
            "3. 【正常回答格式（禁止主動附帶系辦資訊）】：若有參考資料請直接附上資訊，並在回覆最末尾附註參考來源（例如：`📌 參考來源：[來源檔案名稱或網址]`）。\n"
            "**請注意：在能夠正常回答或追問釐清的情況下，絕對不要附帶系辦公室聯絡資訊（電話/信箱/位置等），保持回覆精簡專業**。\n"
            "4. 【益混淆詞彙】請嚴格區分 院訂必修 和 院訂必選"
            "5. 【表格回復禁止】:禁止使用 markdown 表格語法，以條列方式代替"
            "6. 【查無資料/超出範圍處理（僅此情況附帶系辦資訊）】：若參考資料中**完全沒有足夠資訊**回答，或是問題超出資電學士班規章範圍，請明確表示查無相關記載，**僅在此種無法回答的情況下**附上以下系辦公室聯絡方式：\n"
            "   若我有無法回答的問題，或是需要進一步協助，也歡迎透過以下方式聯繫系辦公室：\n"
            f"   🏢 {dept_name}\n"
            f"   📞 電話：{phone}\n"
            f"   📧 信箱：{email}\n"
