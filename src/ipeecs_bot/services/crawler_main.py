...
from .crawlers.table_crawler import TableCrawlerMixin
from .crawlers.text_crawler import TextCrawlerMixin
from .crawlers.web_crawler import WebCrawlerMixin
...
class DataCrawler(WebCrawlerMixin, TextCrawlerMixin, TableCrawlerMixin):
    ...
    def convert_pdf_to_markdown_gemini(
        ...
        prompt = (
            "你是一個專業的國立中央大學學術規章、行事曆與學分學程表格整理專家。"
            "請將這份 PDF 文件完整轉換為結構清晰、語意完整且易於檢索的繁體中文 Markdown 格式。\n\n"
            "轉換要求：\n"
            "1. 完整保留並重現所有表格結構（使用標準 Markdown 表格語法 | 表頭1 | 表頭2 | ...）。\n"
            "2. 若為校曆/行事曆：清楚列出各月份、週次、重要日期、學術活動、註冊選課、開學與放假日程等所有事項。\n"
            "3. 若為課規/章則：清晰列出所有學年度、專長名稱、必修與選修科目名稱、科目代碼、學分數以及畢業修業條件與學分要求規範。\n"
            "4. 完整保留所有附註、備註說明與各項規章要點，不要省略任何一條規則。\n"
            "5. 直接輸出完整的 Markdown 內容，不要包含多餘的聊天對話或開場白。"
        )
    ...
    def crawl_all(
        ...
        for title, url in targets["table_pdf"]:
            if "pdc.adm.ncu.edu.tw" in url and ("1725" in url or "校曆" in title):
                pdfs = self.crawl_latest_academic_calendar(url)
                results["table_pdf_files"].extend(pdfs)
            elif "pdc.adm.ncu.edu.tw" in url:
